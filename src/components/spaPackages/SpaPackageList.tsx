    ...
    <div ref={setNodeRef} style={style}>
      ...
      {/* ================= DESKTOP ROW ================= */}
         <div
          data-row
        className={cn(
          "hidden lg:flex items-center px-4 py-3 mx-3 my-1 rounded-xl gap-2",
      ...
      >
        ...
        <div className="flex-1  text-muted-foreground whitespace-nowrap px-3">
          {item.name}
        </div>
...
const [sortBy, setSortBy] = useState<"id" | "status" | "name" | "index">("index");
...
const handleDragEnd = async (event: DragEndEvent) => {
  ...
  const oldIndex = packages.findIndex((i) => i.id === activeId);
  const newIndex = packages.findIndex((i) => i.id === overId);

  if (oldIndex !== -1 && newIndex !== -1) {
    setPackages((prev) => arrayMove(prev, oldIndex, newIndex));
  }

  try {
   const totalCount = await getTableCount("spa_packages");
const allPackages = await getAllSpaPackages(totalCount);
    const fromIndex = allPackages.findIndex(
      (t) => t.id === activeId
    );
    const toIndex = allPackages.findIndex(
      (t) => t.id === overId
    );

    if (fromIndex === -1 || toIndex === -1) return;

    const reordered = arrayMove(allPackages, fromIndex, toIndex);
    ...
    await reorderSpaPackage(payload);
    fetchSpaPackages();
...
<div className="flex justify-center items-center gap-2 flex-wrap w-full sm:w-auto">
                        ...
                        <button
                            onClick={() => navigate("/admin/packages-list/package-about")}
            className="
            ...
            "
            >
                        <Plus size={16} /> Add Spa Package
                    </button>
</div>
                    </div>
                     <div className="grid grid-cols-12">
                      <div className="col-span-12">
                        <div className="w-full rounded-2xl border border-border bg-card flex flex-col h-[calc(93vh-300px)] md:h-[calc(99vh-300px)]">
                          {/* ================= HEADER (DESKTOP ONLY) ================= */}
